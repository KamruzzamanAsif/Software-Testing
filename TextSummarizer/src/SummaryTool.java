...
class SummaryTool{
	...
	void init(){
		...
		noOfParagraphs = 0;
		try {
	        in = new FileInputStream("samples/nexus_6p.txt");
	        out = new FileOutputStream("samples/output.txt");
	...
	double noOfCommonWords(Sentence str1, Sentence str2){
		...
		String[] sentence1Words = str1.value.split("\\W+");
		String[] sentence2Words = str2.value.split("\\W+");
		for (String word1 : sentence1Words) {
			for (String word2 : sentence2Words) {
